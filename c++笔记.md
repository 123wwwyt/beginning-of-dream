...
把一个三维数组看出一维数组，其实它每一个元素都是二维数组

二维数组不是二级指针，二维指针本质时变量。而二维数组名时二维数组的首地址，也就是一段连续内存空间的首地址，本质上时一个指针常量

结合指针理解，数组名可以理解为数组指针类型的指针常量
...
若：p=a;
p是什么？是数组指针，
首先，p是指针。因为对于数组来说，变量名保存的是
首元素的地址，而二维数组的首元素是一个数组，所以为数组指针
即int (*p)[4]=a;
...
关于a &a *a *(&a)
int *p1 = &a[0][0];
int (*p2)[105] = a;  //这是一个数组指针；
当然：a == &a[0];

int (*p3)[105][105]=&a;
printf("%p %p %p",p1,p2,p3);
输出结果都一样
p1 p2 p3 的关系分别是一个房间，一层，一栋楼
之后：
int b = *p1   是a[0][0]
int *c = *p2    c只能定义一个变量。但总共有105个变量，取不出来，所以只能给出第一个元素的地址，即a[0][0]的地址,则要用指针存
int (*y)[105]= *p3;//取p3只能会取一层的，所以要用一层来存,最后还是对应a
...
看它那个代码
int main(){
    s[i]= ()     //因为这个二级指针已经申请了相应的空间了，所以可以来持续输入
...
结构体是c编程中另一种用户自定义的可用的数据类型，它允许储存不同类型的数据项
...
特殊的
 ...
 type是别名，*type1是结构体的指针，后面再用就不用再用*号了
~~~
### 结构体指针；
...
### 1 函数名是一个指针，
保存函数地址入口，函数名是函数的入口地址，函数的入口地址称为函数指针

### 2函数传参--本质是创建副本

1 实参和形参
调用是实参，声明时是形式参数，如void a[int b]
...
注意：
    1. 最后的\0 最重要，有时输入如果没有也要自己加，但字符串长度不包括这个0
    2. 都是单引号
   
   问题描述：我想输入100位的整数，但如果用long long 最多也就19位。此时只能用字符串输入
   ...
   ~~~
    ...
    int main(){
        ...
        char sb[100]
        //开始存入，安全声明一定要写
        scanf_s("%99s",sb,(unsigned)sizeof(num))
...
2. 字符串变量
    三种形式：
    1. char *str = "hello"
    这个不能修改，因为它初始化指向一个字符串常量，想要修改就用数组，也就算类型2
    2. char word[]="hello"
